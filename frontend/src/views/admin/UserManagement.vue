  ...
  <div>
    <div class="flex justify-between items-end mb-10">
      <div>
        ...
        <p class="text-slate-500 font-medium">Manage students, teachers, and administrators</p>
      </div>
      <button @click="openModal()" class="btn-primary">Add New User</button>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="card bg-white max-w-md w-full shadow-2xl border-white animate-in zoom-in duration-300">
        <h3 class="text-2xl font-black text-slate-900 mb-8">{{ editingUser ? 'Edit User' : 'Add New User' }}</h3>
        <form @submit.prevent="saveUser" class="space-y-5">
          ...
          <div class="flex space-x-3 pt-8">
            ...
            <button type="submit" class="btn-primary flex-grow">Save User</button>
          </div>
        </form>
      </div>
    </div>
